...
// File: @openzeppelin/contracts@4.9.5/utils/Arrays.sol
...
// File: @openzeppelin/contracts@4.9.5/token/ERC20/extensions/ERC20Snapshot.sol
...
// File: @openzeppelin/contracts@4.9.5/utils/math/SafeMath.sol
...
// File: Launcher/StakeTracker.sol
...
contract StakeTracker is ERC20Snapshot {
    ...
    function calc(address user, uint[] memory snapshotIds) external view onlySplitter returns (uint) {
        ...
        for (uint i = 0; i < snapshotIds.length; i++) {
            quantity += getUserReward(user, snapshotIds[i]);
        }
        return quantity;
    }
    ...
    function getUserReward(address user, uint snapshotId) public view returns (uint) {
    ...
    function getReward(uint snapshotId) external view returns (uint) {
        return rewardQuantity[snapshotId];
    }
...
contract Splitter is Rebased, Ownable {
    ...
    function onStake(address user, address token, uint quantity) external onlyRebase {
        if (token == _stakeToken) {
            ...
            if (_startSnapshot[user] == 0) {
                _startSnapshot[user] = _stakeTracker.getCurrentSnapshotId() + 1;
            }
        }
    }
    ...
    function claim(address to, uint limit) external {
        ...
        if (startSnapshot > 0 && startSnapshot <= endSnapshot) {
            if (endSnapshot - startSnapshot > limit) {
                endSnapshot = startSnapshot + limit;
            }
            uint[] memory snapshotIds = new uint[](1 + endSnapshot - startSnapshot);
            for (uint i = startSnapshot; i <= endSnapshot; i++) {
                snapshotIds[i - startSnapshot] = i;
            }
            _startSnapshot[msg.sender] = endSnapshot + 1;
    ...
    /**
     * @dev Returns the amount of unclaimed reward tokens for a specific user.
     * Calculates potential earnings based on stake snapshots since the last claim, up to a limit.
    ...
    function getUnclaimedEarnings(address user, uint limit) external view returns (uint quantity) {
        ...
        if (startSnapshot > 0 && startSnapshot <= endSnapshot) {
            ...
            quantity = _stakeTracker.calc(user, snapshotIds);
        }
    }
